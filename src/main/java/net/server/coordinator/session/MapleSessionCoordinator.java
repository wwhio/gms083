...
import net.vertx.ClientSession;
...
public class MapleSessionCoordinator {
    ...
    private final Map<String, Set<ClientSession>> loginRemoteHosts = new HashMap<>();
    ...
    public static String getSessionRemoteAddress(ClientSession session) {
        return session.getRemoteAddress();
    }

    public static String getSessionRemoteHost(ClientSession session) {
        String nibbleHwid = session.getHWID2();
    ...
    private static MapleClient getSessionClient(ClientSession session) {
        return session.getClient();
    }

    public void updateOnlineSession(ClientSession session) {
    ...
    public boolean canStartLoginSession(ClientSession session) {
        ...
        try {
            ...
            Set<ClientSession> lrh = new HashSet<>(2);
            lrh.add(session);
    ...
    public void closeLoginSession(ClientSession session) {
        String nibbleHwid = session.getHWID2();
        String remoteHost = getSessionRemoteHost(session);

        Set<ClientSession> lrh = loginRemoteHosts.get(remoteHost);
        if (lrh != null) {
            lrh.remove(session);
            if (lrh.isEmpty()) {
    ...
    public AntiMulticlientResult attemptLoginSession(ClientSession session, String nibbleHwid, int accountId, boolean routineCheck) {
        if (!YamlConfig.config.server.DETERRED_MULTICLIENT) {
            session.setHWID2(nibbleHwid);
    ...
    public AntiMulticlientResult attemptGameSession(ClientSession session, int accountId, String remoteHwid) {
        String remoteHost = getSessionRemoteHost(session);
        if (!YamlConfig.config.server.DETERRED_MULTICLIENT) {
        ...
        try {
            String nibbleHwid = session.getHWID2();   // thanks Paxum for noticing account stuck after PIC failure
            if (nibbleHwid != null) {
                onlineRemoteHwids.remove(nibbleHwid);
    ...
    private static MapleClient fetchInTransitionSessionClient(ClientSession session) {
        ...
        if (remoteHwid != null) {   // maybe this session was currently in-transition?
            ...
            if (hwidLen <= 8) {
                session.setHWID2(remoteHwid);
            } else {
                session.setHWID(remoteHwid);
                session.setHWID2(remoteHwid.substring(hwidLen - 8, hwidLen));
            }
    ...
    public void closeSession(ClientSession session, Boolean immediately) {
        MapleClient client = getSessionClient(session);
        if (client == null) {
            client = fetchInTransitionSessionClient(session);
        }

        String hwid = session.getHWID2(); // making sure to clean up calls to this function on login phase
    ...
    public String pickLoginSessionHwid(ClientSession session) {
    ...
    public String getGameSessionHwid(ClientSession session) {
        String remoteHost = getSessionRemoteHost(session);
    ...
    public void printSessionTrace() {
        ...
        if (!loginRemoteHosts.isEmpty()) {
            List<Entry<String, Set<ClientSession>>> elist = new ArrayList<>(loginRemoteHosts.entrySet());

            Collections.sort(elist, new Comparator<Entry<String, Set<ClientSession>>>() {
                @Override
                public int compare(Entry<String, Set<ClientSession>> e1, Entry<String, Set<ClientSession>> e2) {
                    return e1.getKey().compareTo(e2.getKey());
                }
            });

            System.out.println("Current login sessions: ");
            for (Entry<String, Set<ClientSession>> e : elist) {
